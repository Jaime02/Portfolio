...
import HistoriesProgressBar from "@/components/histories/HistoriesProgressBar";
...
const CardsLayout = forwardRef<HTMLDivElement, CardsLayoutProps>(
  ({ children, title, thumbnail, active, isFirstGroup, isLastGroup, selectMyself, goToPreviousHistoryGroup, goToNextHistoryGroup }, ref) => {
    ...
    return (
      <div className={`mx-auto flex h-full w-fit snap-center flex-row items-center gap-4 transition duration-300 ease-in-out ${active ? "" : "scale-50 opacity-50"}`} onClick={onClick} ref={ref}>
