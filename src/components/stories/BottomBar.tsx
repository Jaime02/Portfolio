...
export default function BottomBar({ floatingHeader }: { floatingHeader: boolean }) {
  ...
  return (
    <div
      className={`${floatingHeader ? "absolute bottom-0 z-20" : ""} flex w-full flex-row items-center gap-4 px-4 py-2`}
    >
      <input
        ref={replyInputRef}
        className="border-ig-gray placeholder:text-ig-gray min-w-0 flex-1 rounded-full border-[1px] bg-transparent px-4 py-2 text-white placeholder:text-ellipsis hover:border-white placeholder:hover:text-white focus:border-white focus:placeholder:text-white"
